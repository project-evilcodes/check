...
router.route('/edit-gallery').get(cors(corsOptions), auth.isAuthenticated, (req, res) => {
    if (req.role == 1) {
        let id = req.query.id;
        Gallery.find({_id: id}, (error, data) => {
            if (error) {
                return (error)
            } else {
                res.json(data)
...
router.route('/edit-redhat').post(cors(corsOptions), auth.isAuthenticated, (req, res) => {
    if (req.role == 1) {
        ...
        Redhat.updateOne({_id: id}, {title: title, body: body, date: date}, (error, data) => {
            if (error) {
                console.log(error);
                return (error);
            } else {
...
router.route('/redhat/delete').post(cors(corsOptions), auth.isAuthenticated, (req, res) => {
    if (req.role == 1) {
        let id = req.body.id;
        // delete here
        Redhat.deleteOne({_id: id}).then(res8 => {
            //console.log(res8);
...
router.route('/edit-gallery').post(cors(corsOptions), auth.isAuthenticated, (req, res) => {
    if (req.role == 1) {
        ...
        Gallery.updateOne({_id: id}, {title: title, body: body, date: date}, (error, data) => {
            if (error) {
                console.log(error);
                return (error);
            } else {
